...
const Team = (props) => {
  ...
  const teamMembers = props.teamData.players.map((p, i) => (
        ...
        <Table cards={true} striped={true} responsive={true}>
          <Table.Header>
            <Table.Row>
              ...
              <Table.ColHeader>
                Intercepts: {props.teamData.intercepts}
              </Table.ColHeader>
            </Table.Row>
          </Table.Header>
          <Table.Body>{teamMembers}</Table.Body>
        </Table>
      </Card>
    </Grid.Col>
  );
...
const Teams = (props) => {
  return (
    <Grid.Row cards alignItems="center" className="px-5 mx-5">
      {props.teamsData.map((t, i) => (
        <Team teamData={t} username={props.username} key={i} />
      ))}
    </Grid.Row>
  );
};
